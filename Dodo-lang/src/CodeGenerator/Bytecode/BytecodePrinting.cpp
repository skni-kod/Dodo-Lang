...
std::ostream& operator<<(std::ostream& out, const Bytecode& code) {
    ...
    case Bytecode::AssignAt:
        out << "move value from " << code.op2() << " to address in " << code.op1() << " storing the result in " << code.
            result();
        break;
    ...
    case Bytecode::GreaterEqual:
        out << "check if " << code.op1() << " is equal or greater than " << code.op2() << " stored to " << code.
            result();
        break;
    ...
    case Bytecode::BraceListEnd:
        out << "end of brace list with value passed into: " << code.op3();
        break;
    default:
